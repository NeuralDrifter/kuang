...
export default defineConfig({
  fmt: {
    // The command reference under skills/*/reference is generated by
    // tools/generate-reference.ts, which emits compact markdown tables. The
    // formatter pads those tables to align, the generator un-pads them on the
    // next run, and the two then take turns dirtying 34 files. The generator
    // owns these files; the formatter should not touch them.
    //
    // Compact is also the right output here: these are read by a model through
    // the skill system, where alignment padding is tokens spent on whitespace.
    ignorePatterns: ["skills/*/reference/**"],
  },
  test: {
